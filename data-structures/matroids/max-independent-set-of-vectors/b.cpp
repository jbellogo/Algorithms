...
class Matroid 
{
    ...
    vector <vector<int>> B; // for independence Row reduction check
...
public:
    ...
    void swapRowsA(int i, int j, vector<vector<int>> &A);
...
void Matroid::swapRowsA(int i, int j, vector<vector<int>> &A){
    for (int x = 0; x<m; x++){
        int dummy = A[i][x];
        A[i][x] = A[j][x];
        A[j][x] = dummy;
